...
NOTE: The method sumFirstAndLastDigit needs to be defined as public static like we have been doing so far in the
course.
...
public class FirstLastDigitSum {
    ...
    public static int sumFirstAndLastDigit(int number)
    {
        ...
        if(number < 0)
            return -1;
        int lastNum = number % 10; //Extracting the Last Number using remainder operator.
        int FirstNum;
        while(number >= 10)       // Running loop until the number cuts to the single digit
        {
            number = number / 10;
        }
        FirstNum = number;
